...
flnorm <- function(x, w, b) {w * dlnorm( x+b, 0, 1 )}

fobj <- function(v)
{
    ypred <- flnorm( US$nDays, v[1], v[2], v[3], v[4] )
    err <- abs(ypred - US$New)
    mean(err)
}

res <- optim( c(1,1,1,0), fobj )

param <- as.list(res$par) %>% set_names( c("m", "s", "w","b") )

partial(flnorm, !!!param)(US$nDays)
