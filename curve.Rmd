...
flnorm <- function(x, p) {p[1] * dlnorm(x-p[2], p[3], p[4])}

## Define the starting parameters and fit curves
v0g  <- c(max(US$New)*2, nrow(US), nrow(US)/2)
v0ln <- c(1e5, 0, 5, 1)
cgauss <- fcurve( fgauss, v0g )
clnorm <- fcurve( flnorm, v0ln ) 

## Combine with observed data
P <- bind_rows( mutate(US,          Values="Observed"),
                mutate(cgauss$Pred, Values="Gaussian Fit"),
                mutate(clnorm$Pred, Values="Log-normal Fit") )
