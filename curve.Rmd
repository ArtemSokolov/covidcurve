...
Below, I apply simple curve fitting to model new COVID-19 cases in the U.S. over time. The data is aggregated from three different sources (see below). I chose to fit a log-normal distribution, which is [traditionally used by epidemilogists](https://www.cdc.gov/csels/dsepd/ss1978/lesson1/section11.html) to study cases by date of onset. After fitting the curve, I use it to plot projections for two weeks in the future.
...
Cy <- US %>% filter( Date > ymd("2020-04-05") ) %>% 
  group_by(Week) %>% mutate( WeekOf = as.character(Date[1]) ) %>% 
  ungroup() %>% select(-Week)

ys <- seq(20000, 40000, by=5000)

ggplot( Cy, aes(x=DOW, y=New, group=WeekOf, color=WeekOf) ) + 
  scale_y_continuous(limits=c(15000,40000), breaks=ys, labels=scales::label_number_si()) +
  scale_x_discrete() +
  ...
  theme(axis.ticks.length.y = unit(-5, "cm"), panel.grid=ebl())
